...
Sitios_region_ver <- data_verano %>%
  mutate(Date = as.Date(Date)) %>% 
  group_by(Location, year(Date)) %>%  
  summarise(Censado = n_distinct(Location), .groups = 'drop') %>% 
  mutate(Censado = ifelse(Censado > 0, "*", "")) %>% 
  pivot_wider(names_from = `year(Date)`, values_from = Censado, values_fill = "") %>%  
  rename(`Sitio` = Location) %>%  
  mutate(Total = rowSums(across(`2010`:`2024`, ~ . == "*"))) %>%
  select(`Sitio`, `2010`, `2011`, `2012`, `2013`, `2014`, `2015`, `2016`, `2018`, `2019`, `2023`, `2024`, Total) %>%
  arrange(`Sitio`) %>%  
  gt() %>% 
  data_color(
    columns = Total,  
...
Sitios_region_inv <- data_invierno %>%
  mutate(Date = as.Date(Date)) %>%  
  group_by(Location, year(Date)) %>%  
  summarise(Censado = n_distinct(Location), .groups = 'drop') %>% 
  mutate(Censado = ifelse(Censado > 0, "*", "")) %>%  
  ...
  rename(`Sitio` = Location) %>%  
  mutate(Total = rowSums(across(`2010`:`2024`, ~ . == "*"))) %>%
  select(`Sitio`,  `2009`,`2010`, `2011`, `2012`, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2022`, `2023`, `2024`, Total) %>%
  arrange(`Sitio`) %>%  
  gt() %>% 
  data_color(
    columns = Total, 
